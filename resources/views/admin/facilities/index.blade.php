@extends('admin.layout')

@section('title', 'Facilities Management')
@section('page-title', 'Facilities Management')

@section('content')
<div class="page-header">
    <h1 class="page-title">Facilities Management</h1>
    <p class="page-subtitle">Manage all facilities and amenities of Queen's College</p>
</div>

@if(session('success'))
    <div class="alert alert-success alert-dismissible fade show" role="alert">
        <i class="fas fa-check-circle me-2"></i>
        {{ session('success') }}
        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
    </div>
@endif

<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="card-title">
                <i class="fas fa-building me-2"></i>All Facilities
            </h5>
            <a href="{{ route('admin.facilities.create') }}" class="btn btn-primary">
                <i class="fas fa-plus me-2"></i>Add New Facility
            </a>
        </div>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover data-table" id="facilitiesTable">
                <thead>
                    <tr>
                        <th>Image</th>
                        <th>Name & Description</th>
                        <th>Icon</th>
                        <th>Status</th>
                        <th>Created</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($facilities as $facility)
                    <tr>
                        <td>
                            @if($facility->image)
                                <img src="{{ asset('storage/' . $facility->image) }}" 
                                     alt="{{ $facility->name }}" 
                                     style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;">
                            @else
                                <div class="bg-light d-flex align-items-center justify-content-center" 
                                     style="width: 60px; height: 60px; border-radius: 8px;">
                                    <i class="fas fa-building text-muted"></i>
                                </div>
                            @endif
                        </td>
                        <td>
                            <div>
                                <strong>{{ $facility->name }}</strong>
                                <br>
                                <small class="text-muted">{{ Str::limit($facility->description, 60) }}</small>
                            </div>
                        </td>
                        <td>
                            @if($facility->icon)
                                <i class="{{ $facility->icon }} fa-lg text-primary"></i>
                            @else
                                <span class="text-muted">-</span>
                            @endif
                        </td>
                        <td>
                            <span class="badge bg-{{ $facility->is_active ? 'success' : 'danger' }}">
                                {{ $facility->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td>{{ $facility->created_at->format('M d, Y') }}</td>
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('admin.facilities.show', $facility->id) }}" 
                                   class="action-btn btn-info" 
                                   title="View">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.facilities.edit', $facility->id) }}" 
                                   class="action-btn btn-warning" 
                                   title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.facilities.toggle-status', $facility->id) }}" 
                                      method="POST" style="display: inline;">
                                    @csrf
                                    <button type="submit" 
                                            class="action-btn btn-{{ $facility->is_active ? 'secondary' : 'success' }}" 
                                            title="{{ $facility->is_active ? 'Deactivate' : 'Activate' }}">
                                        <i class="fas fa-{{ $facility->is_active ? 'pause' : 'play' }}"></i>
                                    </button>
                                </form>
                                <form action="{{ route('admin.facilities.destroy', $facility->id) }}" 
                                      method="POST" style="display: inline;" 
                                      onsubmit="return confirm('Are you sure you want to delete this facility?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="action-btn btn-danger" 
                                            title="Delete">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">
                            <i class="fas fa-building fa-3x mb-3 d-block"></i>
                            No facilities found. <a href="{{ route('admin.facilities.create') }}">Add the first facility</a>.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection
